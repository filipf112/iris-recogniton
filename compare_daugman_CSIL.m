...
code1_data = load('D:\Biometria\Iris2\Integrodifferential operator\database2\S2001L01_iris_code.mat');
varName1 = fieldnames(code1_data);
code1 = code1_data.(varName1{1});

% Directory containing the database of iris codes
database_path = 'D:\Biometria\Iris2\Integrodifferential operator\database2\';
...
for t = 1:length(thresholds)
    ...
    correct_recognitions = sum(strcmp(recognized_prefixes, 'S2001L'));
...
for i = 1:length(files)
    ...
    if contains(files(i).name, 'S2001L') % genuine sample
...
disp(['Mean Daugman for CSIL (authentic): ', num2str(mean_authentic)]);
disp(['Variance Daugman for CSIL (authentic): ', num2str(var_authentic)]);
disp(['Mean Daugman for CSIL (impostor): ', num2str(mean_impostor)]);
disp(['Variance Daugman for CSIL (impostor): ', num2str(var_impostor)]);
...
hamming_distance_2 = 0.20:0.005:0.5;
