...
code1_data = load('D:\Biometria\Iris2\Integrodifferential operator\database\S2001L01_iris_code.mat');
varName1 = fieldnames(code1_data);
code1 = code1_data.(varName1{1});

% Directory containing the database of iris codes
database_path = 'D:\Biometria\Iris2\Integrodifferential operator\database\';
...
disp(['Mean Hough for CSIL (authentic): ', num2str(mean_authentic)]);
disp(['Variance Hough for CSIL (authentic): ', num2str(var_authentic)]);
disp(['Mean Hough for CSIL(impostor): ', num2str(mean_impostor)]);
disp(['Variance Hough for CSIL(impostor): ', num2str(var_impostor)]);
